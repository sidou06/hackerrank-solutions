 ...
 * This function calculates the maximum number of handshakes possible 
 * in a room of 'n' people, where each person shakes hands with every other person exactly once.
 */
int handshake(int n) {
    return (n*(n-1))/2; // Using the combination formula C(n,2) = n(n-1)/2
}
...
int main()
{
    ...
    for (int t_itr = 0; t_itr < t; t_itr++) {
        // Read the value of n (number of people in the room)
        char* n_endptr;
        char* n_str = readline();
